...
const Edit = () => {
  ...
  const saveCategory = async (data) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(result.message || "Категория успешно обновлена");
      ...
      <div className="container mx-auto my-10 lg:my-20 px-1 md:px-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start mt-10">
          ...
          <div className="col-span-1 lg:col-span-9 flex flex-col gap-6 shadow-sm p-4">
            <h2 className="subtitle font-playfair">Редактирование категории</h2>
            {loader == true && <Loader />}
            {loader == false && !category && (
              <>
                <Nostate text="Категория не найдена" />
                <Link
                  to="/admin/categories"
