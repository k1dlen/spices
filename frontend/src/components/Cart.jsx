  ...
  return (
    <Layout>
      {/* <div className="container mx-auto my-20 px-1 sm:px-0">
        <h1 className="title text-start mb-20">Корзина</h1>
              ...
              <tr key={item.id} className="border-b border-text-default/20">
                <td className="py-6">
                  <div className="flex items-start gap-4">
                    ...
                    />
                    <div>
                      <h3 className="text-2xl sm:text-3xl md:text-4xl text-text-title font-playfair font-semibold">
                        {item.name}
                      </h3>
                ...
                <td className="py-6 text-center align-top">
                  <span className="text-text-title text-2xl sm:text-3xl md:text-4xl">
                    ₽{item.price.toFixed(2)}
                  </span>
                </td>
                <td className="py-6 text-center align-top">
                  <span className="text-text-default text-2xl sm:text-3xl md:text-4xl">
                    x {item.quantity}
                  </span>
                </td>
                <td className="py-6 text-center align-top">
      ...
      <div className="container mx-auto my-20 px-1 sm:px-0">
        ...
        <div className="lg:hidden flex flex-col gap-6">
          {cartItems.map((item) => (
            <div
              key={item.id}
              className="border border-text-default/20 rounded-md p-4
            ...
            >
              <img
                src={item.image}
                alt={item.name}
                className="w-2/3 sm:w-1/4 h-auto object-contain rounded-md mx-auto sm:mx-0"
              />

              <div className="w-full sm:flex-1 flex flex-col gap-2 text-center sm:text-left">
                <h3 className="subtitle text-text-title font-playfair">
                  {item.name}
                </h3>
              ...
              <button
                aria-label="Удалить из корзины"
                className="text-text-default hover:text-red-500 transition-all duration-300 self-center"
              >
                <FeatherIcon
                  icon="trash"
                  strokeWidth={2.3}
